...
import { Lock, LogOut, Trophy, ShoppingCart, ChevronDown, ChevronUp, Download, Plus, Upload, Trash2, RefreshCw, Calendar, Users, Package } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#19305a] flex items-center justify-center p-4" data-testid="admin-login">
      <div className="bg-white rounded-[24px] shadow-2xl p-8 w-full max-w-sm animate-fade-in">
        <div className="flex flex-col items-center mb-6">
          <div className="w-16 h-16 rounded-full bg-[#19305a] flex items-center justify-center mb-4">
            <Lock size={28} className="text-[#f5a623]" strokeWidth={3} />
          </div>
          <h1 className="text-2xl font-black text-[#19305a]">Admin Access</h1>
          <p className="text-sm text-[#5a6b8a] mt-1">Enter password to continue</p>
        </div>
        <form onSubmit={handleSubmit}>
          <input
            ...
            autoFocus
            className={`w-full px-4 py-3 rounded-[12px] border-2 text-center text-xl font-bold tracking-[0.3em] outline-none transition-all ${
              error ? "border-[#c74747] bg-[#c74747]/5 text-[#c74747]" : "border-[#19305a]/10 bg-[#f3f6fb] text-[#19305a] focus:border-[#7cbde8]"
            }`}
          />
          {error && <p data-testid="admin-login-error" className="text-[#c74747] text-sm font-bold text-center mt-2">Incorrect password</p>}
          <button
            data-testid="admin-login-btn"
            type="submit"
            className="w-full mt-4 h-12 rounded-[12px] bg-[#19305a] text-white font-bold text-lg hover:bg-[#254680] active:translate-y-0.5 transition-all"
          >
            Unlock
          </button>
        </form>
  ...
  return (
    <div className="min-h-screen bg-[#f3f6fb] flex flex-col" data-testid="admin-panel">
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6">
        ...
        {tab === "items" && (
          <div className="animate-fade-in space-y-6">
            <h2 className="text-xl font-bold text-[#19305a]">Shop Items ({items.length})</h2>

            <div className="bg-white rounded-[16px] shadow-[0_8px_24px_rgba(25,48,90,0.06)] p-4">
              <h3 className="text-sm font-bold text-[#7cbde8] uppercase tracking-wider mb-3">Add New Item</h3>
              <div className="flex flex-wrap gap-2">
                <input data-testid="admin-item-name" value={newItem.name} onChange={e => setNewItem(p => ({...p, name: e.target.value}))}
                  placeholder="Item name" className="flex-1 min-w-[150px] px-3 py-2 rounded-[10px] border-2 border-[#19305a]/10 bg-[#f3f6fb] text-sm font-bold text-[#19305a] focus:border-[#7cbde8] outline-none" />
                <input data-testid="admin-item-cost" type="number" value={newItem.cost} onChange={e => setNewItem(p => ({...p, cost: e.target.value}))}
                  placeholder="Cost" className="w-20 px-3 py-2 rounded-[10px] border-2 border-[#19305a]/10 bg-[#f3f6fb] text-sm font-bold text-[#19305a] focus:border-[#7cbde8] outline-none" />
                <select data-testid="admin-item-cat" value={newItem.category} onChange={e => setNewItem(p => ({...p, category: e.target.value}))}
                  className="px-3 py-2 rounded-[10px] border-2 border-[#19305a]/10 bg-[#f3f6fb] text-sm font-bold text-[#19305a] focus:border-[#7cbde8] outline-none">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {items.map(item => (
                <div key={item.name} className="bg-white rounded-[16px] shadow-[0_8px_24px_rgba(25,48,90,0.06)] p-4 flex items-center justify-between group">
                  <div>
                    <div className="font-bold text-[#19305a]">{item.name}</div>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-sm font-bold text-[#f5a623]">{item.cost} tokens</span>
                  ...
                  <button data-testid={`admin-delete-item-${item.name.replace(/\s/g, '-')}`} onClick={() => deleteItem(item.name)}
                    className="w-8 h-8 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 text-[#c74747] hover:bg-[#c74747]/10 transition-all">
