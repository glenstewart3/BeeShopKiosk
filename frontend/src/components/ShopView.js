...
export default function ShopView({ students, items, activeSession, usedPairs, onSave, onSkip }) {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden" data-testid="shop-view">
      {/* Top bar */}
      <div className="px-3 py-1.5 flex items-center justify-between bg-white border-b border-[#19305a]/8 shrink-0">
      ...
      <div className="flex-1 overflow-hidden flex">
        {/* Step 1 - Class */}
        {step === 1 && (
          <div className="flex-1 overflow-y-auto p-3 pt-4 sm:p-4 sm:pt-5 animate-fade-in" data-testid="step-class">
            <h2 className="text-base font-black text-[#19305a] mb-3 tracking-tight">Select a Class</h2>
                ...
                return (
                  <button key={cls} data-testid={`class-btn-${cls}`} onClick={() => handleClassSelect(cls)} disabled={rem === 0}
                    ...
                    <span className={`text-[11px] font-extrabold uppercase tracking-widest ${
                      rem === 0 ? "text-[#19305a]/20" : "bg-[#7cbde8]/12 text-[#7cbde8] px-2 py-0.5 rounded-full"
                    }`}>
                      {rem} remaining
                    </span>
                  </button>
        ...
        {step === 2 && selectedClass && (
          <div className="flex-1 overflow-y-auto p-3 pt-4 sm:p-4 sm:pt-5 animate-fade-in" data-testid="step-student">
        ...
        {step === 3 && (
          <div className="flex-1 flex flex-col p-3 pt-4 sm:p-4 sm:pt-5 animate-fade-in" data-testid="step-tokens">
            <Breadcrumb items={[selectedClass, selectedStudent]} onBack={() => setStep(2)} />
            <div className="flex-1 flex flex-col items-center justify-center">
            <div className="bg-white rounded-[16px] shadow-[0_4px_24px_rgba(25,48,90,0.08)] p-3 w-full max-w-xs flex flex-col border border-[#19305a]/5">
              <h2 className="text-xs font-extrabold text-[#5a6b8a] uppercase tracking-[0.15em] mb-1">Tokens Earned</h2>
              <div data-testid="earned-display" className="text-center text-4xl font-black text-[#19305a] mb-2 min-h-[48px] flex items-center justify-center tabular-nums">
                {earnedBuffer || "0"}
              </div>
              <div className="grid grid-cols-3 gap-1.5 mb-2">
                {[1,2,3,4,5,6,7,8,9].map(d => (
                  <button key={d} data-testid={`numpad-btn-${d}`} onClick={() => keypadInput(String(d))}
                    className="h-11 text-xl font-black rounded-[10px] bg-[#f3f6fb] text-[#19305a] hover:bg-[#7cbde8] hover:text-white active:scale-[0.96] transition-all duration-150 flex items-center justify-center border border-transparent hover:border-[#7cbde8]">
                    {d}
                  </button>
                ))}
                <button data-testid="numpad-btn-clear" onClick={keypadClear}
                  className="h-11 text-xs font-extrabold rounded-[10px] bg-[#c74747]/8 text-[#c74747] hover:bg-[#c74747] hover:text-white active:scale-[0.96] transition-all duration-150">
                  CLR
                </button>
                ...
                <button data-testid="numpad-btn-back" onClick={keypadBack}
                  className="h-11 rounded-[10px] bg-[#f3f6fb] text-[#19305a] hover:bg-[#19305a]/10 active:scale-[0.96] transition-all duration-150 flex items-center justify-center">
                  <Delete size={18} strokeWidth={3} />
                </button>
              </div>
              <div className="grid grid-cols-2 gap-1.5 mb-2">
                <button data-testid="numpad-add-5" onClick={() => keypadAdd(5)}
                  className="h-10 text-base font-bold rounded-[10px] bg-[#7cbde8]/10 text-[#7cbde8] hover:bg-[#7cbde8] hover:text-white active:scale-[0.96] transition-all duration-150">
                  +5
                </button>
              ...
              <button data-testid="set-tokens-btn" onClick={keypadSet}
                className="w-full h-11 text-base font-bold rounded-[12px] bg-[#f5a623] text-[#19305a] shadow-[0_4px_0_rgba(200,130,20,1)] hover:brightness-105 active:translate-y-1 active:shadow-none transition-all duration-150">
                Set Tokens &rarr;
              </button>
            </div>
            </div>
        ...
        {step === 4 && (
          <div className="flex-1 flex overflow-hidden animate-fade-in" data-testid="step-shop">
            {/* Items panel */}
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 border-r border-[#19305a]/6">
                    ...
                    {catItems.map(item => {
                      ...
                      return (
                        <button key={item.name} data-testid={`item-btn-${item.name.replace(/\s/g, '-')}`} onClick={() => addToCart(item)} disabled={disabled}
                          className={`min-h-[68px] rounded-[12px] px-3 py-2.5 font-bold transition-all duration-150 flex flex-col items-center justify-center gap-1 ${
                            disabled
                          ...
                          <span className="truncate max-w-full text-sm font-bold">{item.name}</span>
                          <span className={`text-xs font-extrabold ${disabled ? "text-[#19305a]/15" : "text-[#f5a623]"}`}>{item.cost} tokens</span>
                        </button>
                      );
                    })}
                  </div>
            ...
            {/* Cart panel */}
            <div className="w-56 md:w-64 flex flex-col bg-white shrink-0 border-l border-[#19305a]/5" data-testid="cart-panel">
              <div className="p-2.5 border-b border-[#19305a]/6">
                <div className="flex items-center gap-2 mb-1.5">
                  <ShoppingCart size={14} className="text-[#19305a]" strokeWidth={3} />
              ...
              <div className="flex-1 overflow-y-auto p-2.5 min-h-0">
                {cart.length === 0 ? (
                  <p className="text-[#5a6b8a]/50 text-xs text-center mt-4 font-bold">Tap items to add</p>
                ) : (
                  <div className="flex flex-col gap-1">
                    {cart.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between bg-[#f3f6fb] rounded-[8px] px-2 py-1">
                        <div>
              ...
              <div className="p-2.5 border-t border-[#19305a]/6">
